...
# Version: 5.9.1 (Fully Fixed & Enhanced)
...
function Show-Signature {
    ...
    Write-Host "                              Version: 5.9.1 (Fully Fixed)         " -ForegroundColor Yellow
    Write-Host "                                                                  " -ForegroundColor DarkGray
}
...
function Get-PublicIP {
    try {
        ...
        $publicIP = (Invoke-WebRequest -Uri "https://api.ipify.org" -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop).Content
        if ($publicIP -match '^\d+\.\d+\.\d+\.\d+$') { return $publicIP }
    } catch {}
    try {
        $publicIP = (Invoke-WebRequest -Uri "https://icanhazip.com" -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop).Content.Trim()
        if ($publicIP -match '^\d+\.\d+\.\d+\.\d+$') { return $publicIP }
    } catch {}
    try {
        $publicIP = (Invoke-WebRequest -Uri "https://checkip.amazonaws.com" -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop).Content.Trim()
        if ($publicIP -match '^\d+\.\d+\.\d+\.\d+$') { return $publicIP }
    } catch {}
...
function Install-CustomApp {
    ...
    try {
        ...
        $silentArg = if ($App.SilentArg) { $App.SilentArg } else { "/S" }
        
        $process = Start-Process $installer -ArgumentList $silentArg -Wait -PassThru -NoNewWindow
...
function Get-LatestVersion { param($App)
    ...
    try {
        ...
        if ($r -match "版本:\s*(\S+)") { return $matches[1] }
    } catch {}
    return "Unknown"
}
...
function Show-AllSystemApps {
    ...
    foreach ($line in $lines) {
        $currentLine++
        if ($currentLine % 20 -eq 0) {
            Write-Progress -Activity "Loading Winget apps" -Status "Processing..." -PercentComplete (($currentLine/$totalLines)*100)
        }
    ...
    Write-Progress -Activity "Loading Winget apps" -Completed
...
function Show-MainMenu {
    Clear-Host; Show-Signature
    Write-Host "================================================================================" -ForegroundColor Cyan
    Write-Host "              All-in-One Windows Manager v5.9.1 - by KARIM ABU RIDA" -ForegroundColor White
    Write-Host "================================================================================" -ForegroundColor Cyan
    Write-Host ""
...
if (-not (Test-Admin)) {
    ...
    Write-Host "Some features (updates, activations) may not work properly." -ForegroundColor Yellow
    Write-Host ""
